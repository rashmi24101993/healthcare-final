...
// This method checks if the given doctor exists and executes callback depending on success / failure
async function doctorExists(contract, licenseNumber, success, failure) {
    contract.evaluateTransaction('queryDoctorRecord', licenseNumber).then((response) => {
        if (response) {
            success();
        } else {
            failure();
...
// This method checks if the given patient exists and executes callback depending on success / failure
async function patientExists(contract, aadharNumber, success, failure) {
    contract.evaluateTransaction('queryPatientRecord', licenseNumber).then((response) => {
        if (response) {
            success();
        } else {
            failure();
...
async function invoke( request, socket) {
    ...
    query({
        ...
        fcn: 'queryDoctorRecord',
        args: [args[0]],
    },
    query({
        contract,
        chaincodeId: 'transcrypt',
        fcn: 'queryPatientRecord',
        args: [args[0]],
    },
    socket
    ));
